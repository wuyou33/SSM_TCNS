...
\begin{align}
		...
		&i\in\mathbb{N}_{[1,N]}\}\;,
\end{align}
where $Y_{pq}$ is the component of $Y$ located at row $p$ and column $q$, the notation $\mathtt{proj}_{\mathscr{K}(i)}(\mathbb{R}^n)$ stands for the projection of $\mathbb{R}^n$ onto the subset generated by the components of $x$ indexed by $\mathscr{K}$, $\mathbin{\mathtt{row}}(Y,p)$ is the $p$-th row of $Y$ and $\mathbf{C}$ stands for the subset of $\mathbb{R}^n$ generated by the vector having components zero but those corresponding to the indices defined in $\mathscr{K}$.
...
\mypara In the ``neighbor'' case, the constraint imposed on the matrix $Y$ was that each line of this matrix should depend on the system $i$ and on its neighbors. The the set $\mathscr{K}$ is defined, for each index $i=2,3$, as $\mathscr{K}(i)=\{i-1,i,i+1\}$, $\mathscr{K}(1)=\{1,2\}$ and $\mathscr{K}(4)=\{3,4\}$. The sets $\mathbf{C}$ are defined as
\begin{equation*}
	\begin{array}{rcl}
...
\begin{equation*}
	...
	y_i,&1+\sum_{j\in\mathscr{N}(i)}x_j^2+y_j^2)\end{bmatrix}\;.
\end{equation*}
...
\begin{align*}
 \overline{Y}_i(q_i)=-\begin{bmatrix}2y_i,3+2(x_i^2+y_i^2)\end{bmatrix}
\end{align*}
...
\mypara Solving the matrix inequality~\eqref{eq:Artstein-Sontag:inequality:dual} with ADMM, the components of the matrix $W$ obtained are shown in Equation~?? 
...
\begin{table}
	\begin{tabular}{r|l}
	Method&Time taken by the solver [s]\\ \hline
	Chordal graph decomposition&2.6 to 98\\
	ADMM&
	\end{tabular}
\end{table}
