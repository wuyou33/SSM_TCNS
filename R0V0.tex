...
\mypara The drawback of methods such as CCMs, $H_\infty$ optimization and LQR is that, when the system under consideration has many states (large scale), it does not scale well. This is due to the fact that the stability verification of a linear system with $n$ states may require a function with $n^2$ terms (see \cite{Rantzer2015} for the case of linear systems and Lyapunov functions). In general, the case is worse, for nonlinear systems.
...
\mypara In other words, Problem~\ref{problem formulation} states that the controller is decomposed into smaller components. Each of these depends on the states  prescribed by the set $\mathscr{K}$. The definition of the set $\mathscr{K}$ is not restricted to the topology described by the graph $\mathscr{G}$. Given a graph $\mathscr{G}_c=(\mathscr{V}_c,\mathscr{E}_c)$ describing the communication network among different controllers, the set $\mathscr{K}$ can be defined as a subset of $\mathscr{V}_c$.

\mypara The constraint on the components of $k^\ast$ described in Problem~\ref{problem formulation} is particularly relevant for the design feedback laws with a prescribed structure (topology). Consider the network composed of systems described by the following equation.
...
\mypara The importance of the stability of \eqref{eq:general system:differential} for system \eqref{eq:general system} can be understood as follows. Given a fixed two control $u$ for system \eqref{eq:general system}, $\delta_u\equiv0$. If the Euclidean norm of every solution $\Delta_x(t,x,\delta_x,u,0)$ converges to zero exponentially, as the time tends to infinity, then every pair of solutions to \eqref{eq:general system} converge to each other exponentially. The interested reader may address \cite{Lohmiller1998,Sontag2010} and references therein for further details.
...
\mypara In this paper, these limitations are addressed by imposing a separable structure over $W$.
...
\mypara Following the notation introduced in \cite{Tanaka2011}, for each index $i\in\mathbb{N}_{[1,N]}$, define the set 
\begin{subequations}
	\begin{align*}
		\Xi=\{Y:\mathbb{R}^n\times\mathbb{R}^m\to\mathbb{R}^{m\times n}\ |\mathbin{\mathtt{col}}(Y,j)\in\mathbf{C}_j,\\
		\arg(Y_{ij}(\cdot,u))\in\mathbf{X}(\mathscr{K}(k)),\forall i\in\mathbb{N}_{[1,m]},\forall j\in\mathbb{N}_{[1,n]},k\in\mathbb{N}_{[1,N]}\}\;,
	\end{align*}
\end{subequations}
...
\begin{theorem}\label{thm:main result}
	If there exists a sum-separable control-contraction metric for system \eqref{eq:general system}, then there exists a solution to Problem~\ref{problem formulation}.
\end{theorem}

\mypara The existence of a control contraction metric for system~\eqref{eq:general system} implies that there exist matrices $(W,Y)$ that are a solution to the MI~\eqref{eq:MI formulation} with structure prescribed by the set $\Xi$.
...
\mypara	The MI \eqref{eq:MI formulation} implies that the inequality
	\begin{equation*}
...
\mypara For large scale systems, the dimension of the matrices $W$ and $Y$ can be a problem for the computation of a solution to the MI~\eqref{eq:MI formulation}. This is due to the fact that semidefinite programming become less efficient when those matrices have high rank \cite{VandenbergheBoyd1996}. For this reason, the computation of a solution to MI~\eqref{eq:MI formulation} need to be done in parallel. 

\mypara To that end three methodologies are recalled below: chordal graphs, alternating direction of multipliers (ADMM) and scaled diagonal dominance. The first exploits the network topology to decompose it into smaller connected components. The second method is based on the search of the metric over two sets of matrices: uniformly positive definite matrices and matrices satisfying a specific matrix inequality below. Finally, scaled diagonal dominance exploits the stability of individual agents (i.e., when these considered not connected to the network) and weak coupling. Note that these methodologies can be used either separately or together. The latter would appropriate, when the smaller components of the network have high rank. Some concepts of graph theory and semidefinite programming are recalled from \cite{PakazadHanssonAndersenEtAl2015,VandenbergheAndersen2015,BlairPeyton1993}. 
...
\mypara    Denote the left-hand side of the MI~\eqref{eq:MI formulation} by $T$. The existence of $p$ cliques implies that there exist matrices $F_k:\mathbb{R}^{\dim_k}\to\mathbb{R}^{\dim_k\times \dim_k}$, where $k\in\mathbb{N}_{[1,p]}$, satisfying the equation
   \begin{equation*}
   	 T=\sum_{k=1}^p E_k^\top F_k E_k\;.
  	\end{equation*}
  	
\mypara   	Consequently, the MI~\eqref{eq:MI formulation} can be decomposed into the set of matrix inequalities
...
\mypara Note that if a graph is not chordal, it is possible to make it chordal by adding edges to the graph. The obtained graph is said to be a \emph{chordal embedding}. The above procedure can be applied for the chordal embedded graphs.
...
\mypara The set of scalable diagonally dominant matrices is denoted by $SDD_h$. Let
		\begin{align*}
...
\mypara Whenever the matrix $A$ is scalable diagonally dominant and the choice for $Y$ preserves this structure, the MI~\eqref{eq:MIF} can be relaxed into the following sum-of-squares feasibility program:
\begin{subequations}
...
\begin{align*}
	...
	&B_\perp v\;.
\end{align*}
The feasibility program consists of finding symmetric $Q_W\succeq0$ and $Q_{MI}\succeq0$, called \emph{Gramians}, with appropriate dimensions and such that $p_W=\bar{v}^\top Q_W\bar{v}$ and $p_{MI}=\bar{v}^\top Q_{MI}\bar{v}$ belong to $\mathbb{R}[x,v]$, where $\bar{v}$ is vector of monomials on the variables $x_i,\ldots,x_N$ and $v_1,\ldots,v_N$. 
...
\mypara The MI~\eqref{eq:MI formulation} can be cast into the following optimization problem
\begin{equation*}
	\begin{array}{rl}
...
\mypara Although the requirement of a sum-separable structure seems to be restrictive, this structure is equivalent to the (possibly local) stabilizability of the origin, for some classes of systems.
...
\mypara The graph describing the network structure in provided in Figure~\ref{fig:graph}. Consider the case with $N=4$.
...
\mypara For the remaining cases, the structure of the network composed of systems of the form \eqref{eq:example}. More specifically, Proposition~\ref{prop:clique tree decomposition} can be employed, as the graph describing the interconnection of the network is a subclass of chordal graphs. Consequently, the MI~\eqref{eq:MI formulation} can be decomposed into $N-1$ cliques  described by the matrix
\begin{equation*}
	F_k=\begin{bmatrix}
		F_{11}&F_{12}\\
		F_{12}^\top&F_{22}
...
\mypara The sets $\mathbf{X}(\mathscr{K}(\cdot))$ are defined as $\mathbf{X}(\mathscr{K}(1))=\{q_1,q_2\}$,  $\mathbf{X}(\mathscr{K}(2))=\{q_1,q_2,q_3\}$, $\mathbf{X}(\mathscr{K}(3))=\{q_2,q_3,q_4\}$ and $\mathbf{X}(\mathscr{K}(4))=\{q_3,q_4\}$.
...
\begin{align*}
	Y_{12}(q_1,\breve{q}_1)=&-2.5(1+\sum_{j=1}^2x_j^2+y_j^2)\;,\\	
	Y_{24}(q_2,\breve{q}_2)=&-2.3(1+\sum_{j=1}^3x_j^2+y_j^2)\;,\\	
	Y_{36}(q_3,\breve{q}_3)=&-2.2(1+\sum_{j=2}^4x_j^2+y_j^2)\;,\\	
	Y_{48}(q_4,\breve{q}_4)=&-1.7(1+\sum_{j=3}^4x_j^2+y_j^2)\;.\\	
\end{align*}

\begin{figure*}
\begin{equation}\label{eq:example:Y:neighbor}
	Y(q)=\begin{bmatrix}
	-1.8y_1,&Y_{12}(q_1,\breve{q}_1),&0,&0,&0,&0,&0,&0\\
	0,&0,&-2y_2,&Y_{24}(q_2,\breve{q}_2),&0,&0,&0,&0\\
	0,&0,&0,&0,&-2y_3,&Y_{36}(q_3,\breve{q}_3),&0,&0\\
	0,&0,&0,&0,&0,&0,&-2y_4,&Y_{48}(q_4,\breve{q}_4)
	\end{bmatrix}
\end{equation}
\end{figure*}
 

\mypara In the last case, the fully decentralized scenario has been considered. The time taken by the parser to solve the MI~\eqref{eq:MI formulation} was 2.6 seconds. In this case, $\mathscr{K}(k)=\{k\}$ and $\mathbf{X}(\mathscr{K}(k))=\{q_k\}$, where $k\in\mathbb{N}_{[1,4]}$. Moreover, 
\begin{equation*}
	\begin{array}{rl}
		\mathbf{C}_1=\mathbf{C}_2=\left\{\begin{bmatrix}1,0,0,0\end{bmatrix}^\top\right\},&\mathbf{C}_3=\mathbf{C}_4=\left\{\begin{bmatrix}0,1,0,0\end{bmatrix}^\top\right\},\\
		\mathbf{C}_5=\mathbf{C}_6=\left\{\begin{bmatrix}0,0,1,0\end{bmatrix}^\top\right\},&\mathbf{C}_7=\mathbf{C}_8=\left\{\begin{bmatrix}0,0,0,1\end{bmatrix}^\top\right\}\;.
	\end{array}
\end{equation*}
Solving the MI \eqref{eq:MI formulation} the following matrices $W$ and $Y$ were obtained
\begin{align*}
 W=&\mathbin{\mathtt{diag}}\left(2,1,1,1,1,1,1,1\right),\\
 Y(q)=&\begin{bmatrix}
 	\overline{Y}(q_1)&0&0&0\\
 	0&\overline{Y}(q_2)&0&0\\
 	0&0&\overline{Y}(q_3)&0\\
 	0&0&0&\overline{Y}(q_4)
 \end{bmatrix}
\end{align*}
where
\begin{align*}
 \overline{Y}(q_i)=-\begin{bmatrix}2y_i,3+2(x_i^2+y_i^2)\end{bmatrix}
\end{align*}

\mypara Figure \ref{fig:time graph} shows a plot of the time taken by parser to the MI \eqref{eq:MI formulation} for the three cases considered in this illustration: unconstrained, ``neighbor'' and fully decentralized.
...
\mypara Note that the ADMM cannot be used to solve the matrix inequality~\eqref{eq:Artstein-Sontag:inequality:dual}. The reason is the fact that $B_\perp\equiv0$. 

\begin{figure*}
\begin{align*}
	W_1(q_1)=1\times10^{-7}\begin{bmatrix}
	18-15x_1-28y_1+11.2y_1^2+3.2x_1^2+1.2x_1y_2&9.3+35x_1-43y_1+28y_1^2-16x_1^2-16x_1y_1\\
	*&4.9+40x_1-37y_1+72y_1^2+84x_1^2-155x_1y_1
	\end{bmatrix}\;,\\
	W_2(q_2)=1\times10^{-7}\begin{bmatrix}1-3.7x_2+2.3y_2+1.3y_2^2+3.5x_2^2-4.3x_2y_2&
	-0.6-0.6x_2+y_2+2y_2^2+3.5x_2^2-5.5x_2y_2\\
	*&0.4+2.3x_2-2.3y_2+3.3y_2^2+3.4x_2^2-6.7x_2y_2
	\end{bmatrix}\;,\\
	W_3(q_3)=1\times10^{-7}\begin{bmatrix}
		127-248x_3+18y_3+y_3^2+122x_3^2-18x_3y_3&
		-98+146x_3+19y_3+1.9y_3^2-50x_3^2-22x_3y_3\\
		*&76-78x_3-42y_3+6.4y3^2+21x_3^2+20x_3y_3
	\end{bmatrix}\;,\\
	W_4(q_4)=1\times10^{-6}\begin{bmatrix}
	22+4x_4-17y_4+8.8y_4^2+9.4x_4^2+12x_4y_4&
	-8.6-5.9x_4+1.8y_4-4.2y_4^2-9.7x_4^2-12x_4y_4\\
	*&4.3+7.5x_4+2.8y_4+3.2y_4^2+10x_4^2+11x_4y_4
	\end{bmatrix}\;.
\end{align*}
\end{figure*}
...
\mypara Consider the with four agents (i.e., $N=4$) network having the topology illustrated in Figure~\ref{fig:cycle graph}.
...
\mypara In the ``neighbour'' case, the solver took 11.7s to solve the MI~\eqref{eq:MI formulation} using chordal graphs. The matrix $W$ is given by $\mathbin{\mathtt{diag}}(\overline{W},\overline{W},\overline{W},\overline{W})$, where $\overline{W}=\mathbin{\mathtt{diag}}(0.72,0.19)$. The matrix $Y$ is given by
\begin{equation*}
	Y(q)=\begin{bmatrix}
	...
	\end{bmatrix}\;,
\end{equation*}
where $\underline{Y}(q_i,\breve{q}_i)=-2.26\begin{bmatrix}y_i,&y_i^2+x_i^2+\sum_{j\in\mathscr{N}(i)}y_j^2+x_j^2\end{bmatrix}$,
$\overline{Y}(q_i,\breve{q}_i)=\begin{bmatrix}0.002y_i,0\end{bmatrix}$ and $0_{2\times1}$ stands for the vector $[0,0]\in\mathbb{R}^2$.

\mypara In the ``neighbour'' case, the solver took 5.7s to solve the MI~\eqref{eq:MI formulation} using chordal graphs. The matrix $W$ is given by $\mathbin{\mathtt{diag}}(\overline{W},\overline{W},\overline{W},\overline{W})$, where $\overline{W}=\mathbin{\mathtt{diag}}(0.74,1.05)$. The matrix $Y$ is given by
\begin{equation*}
	Y(q)=\begin{bmatrix}
	...
	\end{bmatrix}\;,
\end{equation*}
where $\overline{Y}(q_i)=-\begin{bmatrix}2.0125y_i,&3.2+2.3(x_i^2+y_i^2)\end{bmatrix}$.

\mypara In the unconstrained case, the solver took 9.8s to solve the MI~\eqref{eq:MI formulation} using chordal graphs. The matrix $W$ is given by $\mathbin{\mathtt{diag}}(\overline{W},\overline{W},\overline{W},\overline{W})$, where $\overline{W}=\mathbin{\mathtt{diag}}(0.69,1.24)$. The matrix $Y$ is given by
\begin{equation*}
	Y(q)=\begin{bmatrix}
	...
	\end{bmatrix}\;,
\end{equation*}
where $\overline{Y}_i(q)=-\begin{bmatrix}2.38y_i,&2.88+\sum_{j=1}^4(x_j^2+y_j^2)\end{bmatrix}$.

\mypara Using ADMM to compute a solution to the MI~\eqref{eq:Artstein-Sontag:inequality:dual}, the solver took 96[s] and the obtained matrix $W$ has the components shown in

\begin{figure*}
	\begin{subequations}
	\begin{equation}\label{eq:}
		\resizebox{.9\textwidth}{!}{$W_1(q_1)=1\times10^{-5}\begin{bmatrix}
		4+3x_1-1\times10^3y_1+1\times10^{5}y_1^2+5x_1^2-1\times10^{3}x_1y_1&2+6x_1-6\times10^{2}y_1^2+3x_1^2-8\times10^{2}x_1y_1\\
		*&2+4x_1+3y_1+3y_1^2+5x_1^2+7x_1y_1
		\end{bmatrix}\;,$}
	\end{equation}
	\begin{equation}\label{eq:}
		\resizebox{.9\textwidth}{!}{$W_2(q_2)=1\times10^{-5}\begin{bmatrix}
		5+9x_2-2\times10^3y_2+1\times10^{5}y_2^2+4x_2^2-1\times10^{3}x_2y_2&6+10x_2-1\times10^{2}y_2-7\times10^{2}y_2^2+5x_2^2-9\times10^{2}x_2y_2\\
		*&7+12x_2+10y_2+4y_1^2+6x_1^2+9x_2y_2
		\end{bmatrix}\;,$}
	\end{equation}
	\begin{equation}\label{eq:}
		\resizebox{.9\textwidth}{!}{$W_3(q_3)=1\times10^{-5}\begin{bmatrix}
		7+16x_3-2\times10^3y_3+1\times10^{5}y_3^2+12x_3^2-2\times10^{3}x_3y_3&10+20x_3-1\times10^{3}y_3+1\times10^{3}y_3^2+11x_1^3-11\times10^{3}x_3y_3\\
...
\mypara In terms of semidefinite programming, employing control-contraction metrics allows one to formulate the search for the controller as a convex optimization problem. Although this is one of its main advantages over the search of control-Lyapunov functions, it does not scale well.
