...
\begin{proof}[Proof of Theorem \ref{thm:main result}]
	...
	It remains to integrate $\delta_k$ to obtain a feedback law for system \eqref{eq:general system} satisfying the constraints of Problem \ref{problem formulation}. Because $M$ is a block-diagonal matrix, the length of any curve $\varphi:[0,1]\to\mathbb{R}^n$ satisfies the following equation
	...
	Note that, due to the structure of the matrix $K$, for each component $i\in\mathbb{N}_{[1,N]}$ of the feedback law $k^\ast$ defined by  Equation~\eqref{eq:contracting feedback law} is given by
	\begin{equation*}
		k_i^\ast(t,s)=u_i^\ast(t)+\int_0^sK_i(\widetilde{\gamma}_i(\sigma),k_i^\ast(t,s))\widetilde{\gamma}_i'(\sigma)\,d\sigma\;,
	\end{equation*}
	where $\widetilde{\gamma}_i(\cdot)\in\mathbf{X}(\mathscr{K}(i))$.
	
	Problem~\ref{problem formulation}. This concludes the proof of Theorem \ref{thm:main result}.
\end{proof}
