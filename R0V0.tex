...
The alternating direction method of multipliers (ADMM) provides a framework to solve the MI~\eqref{eq:MI formulation} in a distributed fashion \cite{BoydParikhChuEtAl2010,MeissenLessardArcakEtAl2015}. To use this framework, define the sets $\mathbf{S}_{\succ0}^n=\{W\in\mathbb{R}^{n\times n}:W\succ0,W=W^\top\}$, $\mathbf{T}=\{Z\in D^{n\times n}:\exists Y\in\mathbb{R}^{m\times n}, T\prec0\}$. For a set $\mathbf{G}$ define the indicator function $H:\{\mathbf{G}\}\times \mathbb{R}^k\to \{0\}\cup\{\infty\}$ as
\begin{equation*}
...
\begin{equation*}
	\begin{array}{rl}
		\text{minimize}&\displaystyle H(\mathbf{T},Z)+\sum_{i=1}^N H(\mathbf{S}_{\succ0}^{n_i},W_i)\\
		\text{subject to}&\mathbin{\mathtt{diag}}(W_1,\ldots,W_N)-Z=0\;.
	\end{array}
\end{equation*}
The ADMM algorithm can be written as
\begin{equation*}
	\begin{array}{rl}
		W_i^{k+1}:=&\mathbin{\mathtt{argmin}}_{W_i}\left(H(\mathbf{S}_{\succ0}^{n_i},W_i))\right)
	\end{array}
\end{equation*}
