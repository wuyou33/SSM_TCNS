...
\begin{abstract}
In this paper, large-scale input-affine nonlinear systems are considered. The problem is the design of distributed controllers for this class of network systems. Although control-Lyapunov functions are the natural candidates to design feedback laws, the computation of these functions is a non-convex optimization problem. Moreover, it may require to verify the positiveness of a function with, at least, $n^2$ elements. In this paper, control-contraction metrics (CCMs) are employed to design controllers with prescribed distributed structures. By using a separable structure, the number of verifications needed can be made smaller, as the search of CCMs is formulated as a distributed semidefinite program. An example illustrates the approach for a network system.
\end{abstract}
...
\mypara Since the vector field $\overline{K}$ is affine on $k^\ast$. This implies that, for every fixed $t\geq0$, and for every $S>0$, there exist constants $c_1(S)$ and $c_2(S)$ such that the inequality
	\begin{equation*}
...
\mypara Figure~\ref{fig:3dsim} shows the $x$-component of a simulation of network~\eqref{eq:example} composed of three systems with the network structure as represented by the graph in Figure~\ref{fig:graph} and with input $u\equiv0$. Note that, when random initial random initial conditions are considered, the system may diverge from the neighborhood of the origin.

\begin{figure}[htpb!]
	\centering
	\includegraphics[width=\linewidth]{./imgs/3dsim.eps}
	\caption{Simulation of network~\eqref{eq:example} for 1s with input $u\equiv0$, $N=3$ and starting from random initial conditions.}
	\label{fig:3dsim}
\end{figure}
...
\mypara In the ``neighbor'' case, the topology of the controller is the same as of the network. More precisely, $\mathscr{G}_c=\mathscr{G}$. The solver took 57s to solve the set of matrix inequalities~\eqref{eq:main result}.
