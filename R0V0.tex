...
\mypara Over the years, methods for the design of distributed controllers for large-scale systems have developed different techniques, according to the vector field that describes the system structure. For linear systems, controller design problems are often translated in terms of optimization.
...
\begin{theorem}\label{thm:main result}
	If there exists a sum-separable control-contraction metric for system \eqref{eq:general system}, then there exists a solution to Problem~\ref{problem formulation}. In particular, the graph $\mathscr{G}_c$ describing the communication network among different controllers specifies the set $\Xi$.
\end{theorem}
...
\mypara 	Note that, due to the constraint $Y\in\Xi$ and the fact that $W$ is diagonal, each component $i\in\mathbb{N}_{[1,N]}$ of the feedback law $k^\ast$, defined by  Equation~\eqref{eq:contracting feedback law}, is given by
	...
	\end{equation*}
	where $\widetilde{\gamma}_i(\cdot)\in\mathbin{\mathtt{proj}}_{\mathscr{K}(i)}(\mathbb{R}^n)$.
	Thus, $k^\ast$ satisfies the constraint imposed in Problem~\ref{problem formulation}. 
	
	To see how the graph $\mathscr{G}_c$ specifies the set $\Xi$, not that the set of edges $\mathscr{E}_c$ specifies the set $\mathscr{K}$ which, in turn, defines the sets $\mathbf{C}$ and $\mathbin{\mathtt{proj}}_{\mathscr{K}}$. This concludes the proof of Theorem~\ref{thm:main result}.
\end{proof}
...
\mypara Figure \ref{fig:time graph} shows a plot of the time taken by parser to the MI \eqref{eq:MI formulation} for the three cases considered in this topology: unconstrained, ``neighbor'' and fully decentralized.  According to this graph shown, for $N=1,2$, the time taken by the parser to solve the MI \eqref{eq:MI formulation} has the same order of magnitude for the three cases. However, as the number of systems increases, the unconstrained case takes more time to be solved than the ``neighbor'' case which, in turn, takes more time than the fully decentralize case. The time difference between the two latter cases can be explained by fact that, for the ``neighbor'' case, the matrix $Y$ contains more non-identically zero elements than the fully decentralized case. In addition to this, in the former case, each element is defined as a polynomial of second degree on the variables $q_i$ and $\breve{q}_i$, while for the latter, they are defined on the variable $q_i$ only.
...
\mypara Consider the with network with four agents (i.e., $N=4$) with the topology illustrated in Figure~\ref{fig:cycle graph}. Also in this topology, chordal graphs and ADMM are employed to solve the MIs~\eqref{eq:MI formulation} and \eqref{eq:Artstein-Sontag:inequality:dual}, respectively.
...
\mypara In the ``unconstrained'' case, the solver took 7.4s to solve the MI~\eqref{eq:MI formulation} using chordal graphs. The matrix $W$ is given by $\mathbin{\mathtt{diag}}(\overline{W},\overline{W},\overline{W},\overline{W})$, where $\overline{W}=\mathbin{\mathtt{diag}}(0.7,1.2)$. The matrix $Y$ is given by
\begin{equation*}
	Y(q)=\begin{bmatrix}
...
\begin{equation*}
	...
	\end{bmatrix}\;,
\end{equation*}
where $\underline{Y}(q_i,\breve{q}_i)=-2.26\begin{bmatrix}y_i,&1+\sum_{j=1}^4y_j^2+x_j^2\end{bmatrix}$,
...
\begin{equation*}
	...
	\end{bmatrix}\;,
\end{equation*}
where $\overline{Y}_i(q)=-\begin{bmatrix}3.1y_i,&2.3+2.3\sum_{j=1}^4(x_j^2+y_j^2)\end{bmatrix}$.
...
\mypara In terms of semidefinite programming, employing control-contraction metrics allows one to formulate the search for the controller as a convex optimization problem. Although this is one of its main advantages over the search of control-Lyapunov functions, it cannot handle systems with a large number of systems, in general.
