...
\mypara As $k\to\infty$, for each index $i\in\mathbb{N}_{[1,N]}$, the Gramians $Q_{W_i}$ will coincide with the matrices $Z_i$. Consequently, the block-diagonal matrix $Z$ is symmetric and positive definite. Thus, $p_{MI}\in\mathbb{SOS}[x,v]$ and, for each index $i\in\mathbb{N}_{[1,N]}$, $v_i^\top W_iv_i\in\mathbb{SOS}[x_i,v_i]$. Hence, $M=\mathbin{\mathtt{diag}}(W_1,\ldots,W_N)^{-1}$ is a separable control contraction metric for system \eqref{eq:general system}.
