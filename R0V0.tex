...
\begin{equation}\label{eq:Artstein-Sontag:inequality:dual}
 	\ker(B)^\top\left(-\dot{W}+AW + WA^\top+2\lambda W\right)\ker(B)\prec0
\end{equation} 
holds, where $\ker(B)$ is a full-rank matrix satisfying $B^\top\ker(B)\equiv0$. 
...
\begin{align*}
	p_W=&\ v^\top W v\;,\\
	p_{MI}=&\ -v^\top\ker(B)^\top(-\dot{W}+AW + WA^\top+2\lambda W)\\
	&\ker(B)v\;.
\end{align*}
...
\mypara Using ADMM to solve the matrix inequality~\eqref{eq:Artstein-Sontag:inequality:dual},  the solver took 23 seconds to compute the matrix $W$ for system~\eqref{eq:example} with four agents lead to the 
...
\mypara In this work, input-affine nonlinear systems were considered. The problem was the design of structured feedback laws. The methodology employed for synthesis was based on control-contraction metrics which provides a controller ensuring exponential convergence of pairs of solutions.
