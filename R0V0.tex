...
The parser took 98  seconds to solve the MI~\eqref{eq:MI:SSCCM}. The matrix $W$ is given by
\begin{equation*}
...
\mypara Consider the network with four agents (i.e., $N=4$) and the topology illustrated in Figure~\ref{fig:cycle graph}. Also here, chordal graphs are employed to solve the MIs\eqref{eq:MI formulation}.
...
\begin{table}
	...
	&7.7s (neighbor)\\
	&6.2s (decentralized)\\\hline
	\end{tabular}
	\caption{A summary of the time taken to solve the matrix inequality~\eqref{eq:MI formulation} using chordal graph decomposition, in the topology illustrated by Fig.~\ref{fig:cycle graph}.}
