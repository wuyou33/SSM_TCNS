...
System \eqref{eq:example} is a variation of the ``Goodwin model'' (introduced in \cite{Goodwin1965}) and considered in \cite{Aminzare2014a}. When $d=0$, this system is a generic model describing how persistent oscillations can be generated by an autoinhibitory gene circuit \cite[Section 7.3.1]{Ingalls2013}. The variable $x_i$ is the mRNA is translated into the enzyme $y_i$ which catalyses the production of the metabolite $z$. The metabolite $z$ supresses the transcription of the mRNA. When $d\neq0$, the composed network is interconnected throughout a diffusive term.
