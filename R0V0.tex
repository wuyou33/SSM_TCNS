...
Following the notation introduced in \cite{Tanaka2011}, For each index $i\in\mathbb{N}_{[1,N]}$, define the sets 
\begin{subequations}
	\begin{align*}
		\Xi=&\ \{Y:\mathbb{R}^n\times\mathbb{R}^m\to\mathbb{R}^{m\times n}\ |\mathbin{\mathtt{col}}(Y,j)\in\mathbf{C}_j,\\
		&\ \forall j\in\mathbb{N}_{[1,N]}\}\;,
	\end{align*}
\end{subequations}
where $\mathbin{\mathtt{col}}(Y,j)$ stands for the $j$-th column of the matrix $Y$, and the set $\mathbf{C}_j$ is the space spanned by the sparsity vector $v_j\in\mathbb{R}^{n}$. The set $\Xi$ defines the topology of the differential feedback law to be designed for system~\eqref{eq:general system:differential}. 

It remains to specify the dependence of each component of the matrix $Y$ on the elements of the state-space variable. To that end, let the set
\begin{align*}
		\mathbf{Y}_i=\{Y_{ij}:\mathbb{R}^n\times\mathbb{R}^m\to\mathbb{R}^{m\times n}\ | \arg(Y_{ij})\in\mathbf{X}_i\}\;,
\end{align*}
where the set $\mathbf{X}_i$ is the space spanned by components of $x$ defined by the set $\mathscr{K}(i)$.
...
Consider a graph $\mathscr{G}=(\mathscr{V},\mathscr{E})$ describing the topology of the network \eqref{eq:general system}. A \emph{clique} $\mathscr{C}\subset\mathscr{V}$ of the graph $\mathscr{G}$ is the maximal set of nodes that induces a complete subgraph on $\mathscr{G}$. A \emph{clique tree} is a tree that has cliques as its nodes. An algorithm to compute the number of cliques is provided in \cite{BronKerbosch1973}.
