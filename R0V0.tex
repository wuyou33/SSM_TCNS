...
\mypara The alternating direction method of multipliers (ADMM) provides a framework to compute the block-diagonal matrix $W$ to satisfy the MI~\eqref{eq:MI formulation} in a distributed fashion \cite{BoydParikhChuEtAl2010,MeissenLessardArcakEtAl2015}. To use this framework, define the sets $\mathbf{S}_{\succ0}^n=\{W\in\mathbb{R}^{n\times n}:W\succ0,W=W^\top\}$, $\mathbf{T}=\{W_1,\ldots,W_N:\exists Y\in\Xi,(\mathbin{\mathtt{diag}}(W_1,\ldots,W_N),Y)\text{ is a solution to \eqref{eq:MI formulation}}\}$. For a set $\mathbf{G}$ define the indicator/barrier function $H:\{\mathbf{G}\}\times \mathbb{R}^k\to \{0\}\cup\{\infty\}$ as
\begin{equation*}
...
\begin{equation*}
	\begin{array}{rl}
		\text{minimize}&\displaystyle H(\mathbf{T},(Z_1,\ldots,Z_N))+\sum_{i=1}^N H(\mathbf{S}_{\succ0}^{n_i},W_i)\\
		\text{subject to}&W_i-Z_i=0,\quad\forall i\in\mathbb{N}_{[1,N]}\;.
	\end{array}
\end{equation*}
...
\begin{enumerate}
	...
	\begin{equation*}
	\begin{array}{rcl}
		W_i^{k+1}&=&\mathbin{\mathtt{argmin}}_{W_i\in\mathbf{S}_{\succ0}^{n_i}}\left|\left|W_i-Z_i^k+\Lambda_i^k\right|\right|_F^2\;,
	\end{array}
\end{equation*}
	...
	\begin{equation*}
		Z^{k+1}=\mathbin{\mathtt{argmin}}_{Z\in\mathbf{T}}\sum_{i=1}^N\left|\left|W_i^{k+1}-Z_i+\Lambda_i^k\right|\right|_F^2\;;
	\end{equation*}
	...
	\end{equation*}
\end{enumerate}
Note that steps 1 and 3 are computed in parellel while step 2 requires to gather all the values of the matrices $W_i$ and the values $\Lambda_i$.
