...
\mypara Inequality~\eqref{eq:Artstein-Sontag:inequality} is satisfied, whenever the matrix inequality
...
\mypara Once $Q$ has been computed, employing Finsler's lemma \cite[Lemma~11.1]{CalafioreGhaoui2014} pointwisely, the existence of a function $Y:\mathbb{R}^n\to\mathbb{R}^{n\times m}$ satisfying MI~\eqref{eq:MI formulation} is ensured.
...
\mypara A case where structured controller design is relevant is on distributed design for network systems. Let $N>0$ be a constant integer, consider the graph describing the network structure in Figure~\ref{fig:graph}.
\begin{figure}[htpb!]
	\centering
