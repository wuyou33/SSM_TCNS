...
Consider the graph $\mathscr{G}=(\mathscr{V},\mathscr{E})$ describing the topology of the network~\eqref{eq:general system}. Let $k\in\mathbb{N}_{[1,N]}$ be a constant value and $\{1,\ldots,k\}$ be a path. A \emph{chord} of a path is any edge joining two nonconsecutive nodes.  
