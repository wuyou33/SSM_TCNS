...
\begin{align*}
	\ell(\gamma)=&\ \int_0^1\left|\gamma_s(s)\right|_{\gamma(s)}\,ds\\
	e(\gamma)=&\ \int_0^1V\left(\gamma(s),\gamma_s(s)\right)\,ds\;,
\end{align*}
where the notation $\gamma_s$ stands for the derivative $\tfrac{\partial\gamma}{\partial s}$. The Riemannian distance between $x_0$ and $x_1$, denoted as $\mathbin{\mathtt{dist}}(x_0,x_1)$, is defined as the curve with the smallest length connecting them. This curve is said to be a \emph{geodesic} and it is the solution to the optimization problem
...
\begin{equation}\label{eq:contracting feedback law}
	k^\ast(t,\sigma)=u^\ast(t)+\int_0^\sigma K(\gamma(s),k^\ast(t,s))\gamma_s(s)\,ds\;,
\end{equation}
...
\begin{proof}[Proof of Theorem \ref{thm:main result}]
	...
	It remains to integrate $\delta_k$ to obtain a feedback law for system \eqref{eq:general system} satisfying the constraints of Problem \ref{problem formulation}. Because $M$ is a block-diagonal matrix, the energy of any curve $\varphi:[0,1]\to\mathbb{R}^n$ satisfies the following equation
	\begin{equation}\label{eq:geodesic sum}
		e(\varphi)=\int_0^1\sum_{i=1}^N V_i\left(\varphi_i(s_i),\frac{\partial \varphi_i}{\partial s_i}(s_i)\right)\,ds\;.
	\end{equation}
	...
	\begin{equation*}
		\gamma_i=\arg\inf e(\varphi_i)\Leftrightarrow\gamma=\arg\inf\ell(\varphi),\quad\forall i\in\mathbb{N}_{[1,N]}\;.
	\end{equation*}
	...
	\begin{equation}\label{eq:K ODE}
		\frac{dk^\ast}{ds}(t,s)=K(\gamma(s),k^\ast(t,s))\gamma_s(s)=:\overline{K}(t,s,k^\ast)\;.
	\end{equation}
	...
	\begin{equation*}
		k_i^\ast(t,s)=u_i^\ast(t)+\int_0^sK_i(\widetilde{\gamma}_i(\sigma),k_i^\ast(t,s))\frac{\partial\widetilde{\gamma}_i}{\partial \sigma}(\sigma)\,d\sigma\;,
	\end{equation*}
...
Before showing how the MI \eqref{eq:MI formulation} can be computed in parallel, some concepts of graph theory and semidefinite programming are recalled from \cite{PakazadHanssonAndersenEtAl2015,VandenbergheAndersen2015,BlairPeyton1993}. 

Consider the graph $\mathscr{G}=(\mathscr{V},\mathscr{E})$ describing the topology of the network~\eqref{eq:general system}. Let $k\in\mathbb{N}_{[1,N]}$ be a constant value and $\{1,\ldots,k\}$ be a path. A \emph{chord} of a path is any edge joining two nonconsecutive nodes. A graph is said to be \emph{chordal} if every cycle greater than three has a chord. 

A \emph{clique} $\mathscr{C}\subset\mathscr{V}$ of the graph $\mathscr{G}$ is the maximal set of nodes that induces a complete subgraph on $\mathscr{G}$. 
