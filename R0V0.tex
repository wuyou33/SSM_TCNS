...
\mypara Solving the matrix inequality~\eqref{eq:Artstein-Sontag:inequality:dual} with ADMM, the components of the matrix $W$ obtained are shown in the set of Equations~\eqref{eq:ADMM:chain case}. A summary of the time taken by the solver employing chordal graph and ADMM approach is shown in Table~\ref{tab:chain case}.
...
\begin{table}
	...
	ADMM&300s\\\hline
	\end{tabular}
	\caption{A summary of the time taken to solve the matrix inequalities \eqref{eq:MI formulation} and \eqref{eq:Artstein-Sontag:inequality:dual} using chordal graph decomposition and ADMM approaches, respectively, in the case of the chain topology}
...
\mypara Using ADMM to compute a solution to the MI~\eqref{eq:Artstein-Sontag:inequality:dual}, the solver took 96[s] and the obtained matrix $W$ has the components shown in the set of Equations~\eqref{eq:ADMM:cyclic case}.
