...
Let $N>0$ be an integer, for each index $i\in\mathbb{N}_{[1,N]}$, system~\eqref{eq:general system} can be decomposed into smaller components as follows.
...
The MI~\eqref{eq:MI formulation} can be cast into the following optimization problem
\begin{equation*}
	\begin{array}{rl}
...
In the last case, the fully decentralized scenario has been considered. The time taken by the parser to solve the MI~\eqref{eq:MI formulation} was 2.6 seconds. In this case, $\mathscr{K}(k)=\{k\}$ and $\mathbf{X}(\mathscr{K}(k))=\{q_k\}$, where $k\in\mathbb{N}_{[1,4]}$. Moreover, 
\begin{equation*}
	\begin{array}{rl}
