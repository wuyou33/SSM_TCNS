...
Let $\mathscr{C}$ be a clique of the graph $\mathscr{G}$ and consider the corresponding matrix $F$ that must satisfy the MI~\eqref{eq:MIF}. If the matrix $F$ has some structure on its diagonal, then the negative definiteness can be shown using the optimization method proposed in \cite{AhmadiHall2016}.
...
The set of scalable diagonally dominant matrices $D\in\mathbb{R}^{h\times h}$ described in Definition~\ref{def:SDD} is denoted by $SDD_h$. Let
		\begin{equation}\label{eq:}
			\mathbf{SDD}(U)=\{P\succ0,P\in\mathbb{R}^{h\times h}:P=UDU, D\in SDD_h\}
		\end{equation}
be the set of positive definite matrices parameterized by a matrix $U\in\mathbb{R}^{h\times h}$. If the MI~\eqref{eq:MIF} can be casted into the following second-order cone programming
\begin{subequations}
